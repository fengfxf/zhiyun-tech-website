...
### 需求说明
1. **功能描述**：
   - 在首页主标题下方（红色框区域），添加一个从右到左滚动的AI业界新闻条。
   - 新闻内容为模拟数据，后续可对接Agent自动抓取真实新闻。
   - 新闻条可自动循环滚动，速度适中（不宜过快）。
   - 支持点击某条新闻，跳转到对应的外部新闻详情页面。
   - 支持暗黑/明亮模式，字体颜色自动适配背景。
...
3. **样式要求**：
   - 新闻条高度适中，内容居中显示。
   - 字体在暗黑模式下为浅色，明亮模式下为深色。
   - 新闻之间用分隔符（如"|"）分隔。

4. **数据来源**：
   - 目前为前端模拟AI新闻数据，后续可通过API/Agent自动更新。

5. **参数说明**：
   - 新闻数据结构：`[{ title: string, url: string }]`
   - 滚动速度可配置。
